# ConvertBatchVolumeMax.ps1
# Normalizuje k nejhlasitějšímu souboru (max_volume), zachovává strukturu, převádí a přesouvá originály do Done.
...
foreach ($file in $files) {
    ...
    # Run ffmpeg volumedetect and capture stderr
...
# Determine target: highest (max) peak among files
$targetVolume = ($volumeMap.Values | Measure-Object -Maximum).Maximum
Log ("Target peak (max of all files): {0} dB" -f ([math]::Round($targetVolume,2))) "Blue"
...
foreach ($file in $files) {
    ...
    $gainDb = [math]::Round($targetVolume - $currentVolume, 2)

    if ($gainDb -eq 0 -or $currentVolume -eq -999.0) {
        Log "Converting without volume change: $($file.Name)" "Blue"
        $audioFilterArgs = @()
    } else {
        Log "Converting with gain ${gainDb} dB: $($file.Name)" "Blue"
        # use -filter:a "volume=XdB"
        $audioFilterArgs = @("-filter:a", "volume=${gainDb}dB")
    }

    if ($DoConvert) {
        # Build ffmpeg args array for safer invocation
